...
| **lens_ps_china_pools** (BE-5, cip_162) | Per brand: its CDP declaration POOL (the four-pool china-declaration pipeline: confirmed / just-pay / investigate). Drives the /pools collection queues. Columns: `wayward_brand_id`, `pool`. |
...
| **lens_ps_wayward_acknowledgment** (BE-2) | Per brand: has Wayward ACKNOWLEDGED/paid it (does it appear in `ps_payment_events`), plus latest payment date, total paid, and payment count. A brand ABSENT here has never been acknowledged (the reconciliation dispute = china brands with a positive `lens_ps_claim.mgmt_fee_owed` absent here). |
| **lens_ps_wayward_indicators** (BE-3, cip_160) | Per brand: do Wayward-adjacent sources INDICATE china (a `points_to='china'` signal in `ps_nationality_signals` from a Wayward/HubSpot/exclusion-list/eric-sheet/wechat/amazon source), plus the source list and best strength. Evidence only; the verdict is `lens_ps_china_verdict`, not this. |
