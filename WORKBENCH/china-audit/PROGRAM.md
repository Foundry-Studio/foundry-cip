...
| P4 | Reporting Frontend (was "Metabase Dashboards") | `b3efe08b` (WCC4) | **active — CORRECTED 2026-07-22; plan of record [REPORTING-REBUILD-PLAN.md](REPORTING-REBUILD-PLAN.md)** | **PIVOT 2026-07-18:** custom Next.js frontend on the lenses; retire Metabase for PS money reporting. A **Phase-0 app SHIPPED** (live/deployed/authed/i18n at reports.project-silk.com) but **drifted to role-home dashboards** — built off the design handoff, NOT the operational-pipeline plan, and **never PM-kicked-off** (3 plan docs went unreconciled). **Re-anchored 2026-07-22** to the "follow-a-dollar" pipeline (Pipeline Overview · Revenue/Billing · Collections · What-Wayward-Owes+recon · Payments-In · Partners · Brand&Product · Brand-360 · Exceptions · **Statements** · Partners-write · **User/Access Admin + Activity Log**). Design system kept; 6 money-labeling defects + the RBAC default fixed; external Wayward/partner surfaces = data-gated V2 backlog. Now rebuilt in PM with sprints (task-per-screen). |
...
| date | decision |
|------|----------|
| 2026-07-22 | **P4 reporting re-anchored + corrected.** The shipped Phase-0 app drifted from the operational recovery pipeline to role-home dashboards (built off the design handoff, not [REPORTING-FRONTEND-PLAN.md]; never PM-kicked-off; 3 plan docs unreconciled). Plan of record = **[REPORTING-REBUILD-PLAN.md](REPORTING-REBUILD-PLAN.md)** (supersedes the role-home screen set in REPORTING-BUILD-PLAN.md; re-instates the operational pipeline from REPORTING-FRONTEND-IMPLEMENTATION.md). Decisions (Tim): write-surface = **governed FAS API**; Partners = one page + light write; raw GMV = **derived**; external Wayward view = **deferred (PM backlog)**; access = **role-based per-person, multi-admin, add-by-email, ungranted pages hidden/404, no money-gate**; **full activity/usage logging + admin Report Builder** (append-only Postgres). Rebuilt in PM with sprints. |
