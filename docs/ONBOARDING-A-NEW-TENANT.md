...
### Phase 0 — Stakeholder alignment + tenant UUID resolution (Tim + Atlas + new tenant lead)

**Canonical UUID rule (added 2026-05-16):** new tenants MUST be created in the JOS `tenants` table FIRST, with a real UUIDv4 generated by JOS provisioning. **Never use placeholder UUIDs** like `b0000000-...0001` even as a "working stand-in" — the 2026-05-12 Wayward placeholder reservation later required a full data-migration (1.26M rows across 14 tables; see `cip_12_seed_wayward_client` + `scripts/migrate_b0_to_ecomlever.py`) to correct.

**Tenant vs. client model** (per VISION §4):
- **Tenant** = the venture/operator (EcomLever, Project Silk, Rocky Ridge, Foundry-self). Row in `tenants` table.
- **Client** = the subject-of-intelligence inside a tenant (Wayward inside EcomLever; future PS clients inside Project Silk). Row in `cip_clients`.
- **Lens** = the filtered perspective on top (e.g., "China-only view of Wayward"). Row in `cip_views`.

For a brand-new tenant, both a `tenants` row AND a `cip_clients` row may need creating. For an existing tenant adding a new client, only the `cip_clients` row. The client_id is best generated as a deterministic UUIDv5 derived from `uuid5(tenant_id_uuid, client_slug)` so it's reproducible across environments.
