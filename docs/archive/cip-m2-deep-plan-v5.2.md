---
id: CIP-SPEC-902
uuid: 37e22598-b8b8-4a1b-ad0e-2235dbfb615b
title: CIP Phase 1 — Milestone 2 Deep Implementation Plan
type: spec
owner: Atlas (author) / Claude Code (executor)
solve_for: Retired/archived artifact retained for audit and historical context — cip-m2-deep-plan-v5.2.md.
stage_label: retire
domain: meta
version: v5
created: '2026-05-05'
...
>
> **Status:** v5 draft, after Round-4 LLM panel (7 expert models on the v4 plan — 3 SEV-5 mandatory + 3 SEV-3-4 strongly-recommended fixes folded in). Next: Claude Code handoff for foundry-cip extraction (Stage 1) → M2 implementation.
>
> **Precondition:** Tasks #57–#65 (foundry-cip repo extraction) MUST land before M2 executes. All paths assume the post-extraction layout.
>
...
>
> 4. **PATCH-Q6 (SEV-3, strongly-recommended) — `KnowledgeText.metadata` becomes `KnowledgeTextMetadata` TypedDict.** D-133 amended 2026-04-29 (see DECISION-LOG.md). 6 of 7 panel models flagged `metadata: dict[str, object]` as a semver hole at the framework boundary. New shape: `class KnowledgeTextMetadata(TypedDict)` with required core keys (`source_id`, `source_system`, `extracted_at`, `tenant_id`, `connector_version`) plus `NotRequired` extensions (`authority`, `record_updated_at`, `ingestion_batch_id`). Connectors extend via TypedDict subclassing for source-specific fields. mypy-strict catches typos in metadata keys at CI time. M2 MockMapper's `metadata={}` becomes `metadata={"source_id": …, "source_system": "fixture", "extracted_at": _utcnow(), "tenant_id": tid, "connector_version": "0.0.0"}`. See §4.1 patch + DECISION-LOG.md D-133 amendment.
>
...
**Repo target:** `foundry-cip` (separate repo under Foundry-Studio org, extracted before M2). Per D-118, connector framework lives under `platform/integration-mesh/` semantically, which within `foundry-cip` maps to `cip/integration_mesh/` (final path TBD in extraction tasks #60–#61).
...
**Path-inference note:** If the final path under `foundry-cip/` ends up being `cip/integration_mesh/` (as drafted above) vs `cip/connectors/` or `cip/mesh/`, the SPEC §3 rewrite in task #60 sets the source of truth. This plan uses `cip/integration_mesh/` as a working placeholder.
