...
>
> **v5.4 revision summary (2026-05-06) — POST-IMPLEMENTATION PLAN-HYGIENE PASS:** M2 shipped in foundry-cip at HEAD `99d8ca0` on 2026-05-06 with 154 passing tests, 94% coverage, 6/6 CI matrix green, and 19 of 22 §9 acceptance criteria PASS (3 documented deferrals). During the build, 14 plan-vs-reality reconciliations surfaced — places where the v5.3 plan text disagreed with the deployed `cip_01`–`cip_08` migrations or with Postgres SQL grammar. CC adapted to the deployed truth at every reconciliation point per P-22 / D-123 (migrations are the authority); v5.4 is the matching plan-document update. **No architecture change.** No code change post-M2. Plan text now matches deployed code.
>
> **The 14 reconciliations + 3 process-text fixes captured in v5.4:**
>
> 1. **Δ1 (§4.1)** — `_assert_tz_aware` signature widened from `value: datetime` → `value: object`. The helper is called from `dict[str, object]` callsites; the narrow annotation makes mypy strict flag the runtime `isinstance` guard as unreachable. Behavior preserved.
> 2. **Δ2 (§4.7)** — `cip_sync_runs` 5-counter shape canonical. Pre-v5.4 plan specified 7 SQL counters + a `metadata` → `run_metadata` rename gated on two new migrations (`cip_11`, `cip_12`); these were never authored. Deployed `cip_03_sync_runs` ships 5 counters (`rows_ingested`, `rows_history`, `rows_created`, `rows_updated`, `rows_skipped`) + a `metadata` column. v5.4: keep the granular 7 in-memory for structured logging; map 7 → 5 at SQL write time. No new migrations gate M2.
> 3. **Δ3 (§4.5)** — bitemporal SCD-2 history shape canonical. Pre-v5.4 plan used a single `archived_at` timestamp + `id` + `client_id` snapshot. Deployed `cip_*_history` tables (all 7) use bitemporal SCD-2: `record_id` (FK), `valid_from`, `valid_to`, `changed_by`, `change_reason`. The bitemporal shape is genuinely better than the plan's model (matches Kimball SCD-2 norms).
> 4. **Δ4 (§4.5)** — per-table `EXTRAS_COLUMN_BY_TABLE` mapping. Pre-v5.4 plan used a uniform overflow JSONB column. Deployed reality is heterogeneous: `cip_clients` uses `metadata`, `cip_views` has none, the other 5 use `properties`.
> 5. **Δ5 (§4.5)** — `cip_views` overflow guard. `cip_views` has no extras column; persister raises `PersistenceError` when a mapper produces overflow keys for it. Fail-loud catches mapper bugs the moment a real connector hits the path.
> 6. **Δ6 (§4.5)** — `source_id IS NOT DISTINCT FROM :source_id` for nullable natural keys. `cip_files.source_id` is nullable; the standard equality lookup is always-false against NULL rows. Uniform `IS NOT DISTINCT FROM` across all 7 tables.
> 7. **Δ7 (§4.8)** — `_utcnow()` defined as orchestrator-private helper. Pre-v5.4 plan referenced `_utcnow()` in the metadata-finalize block but never defined it.
> 8. **Δ8 (§4.8)** — setdefault semantics split. Three keys (`source_system`, `connector_version`, `extracted_at`) keep `setdefault` (mapper-may-know-better); two keys (`tenant_id`, `ingestion_batch_id`) use **detect-then-assign** (orchestrator-owned; raise `KnowledgeMetadataValidationError` on mapper-emitted mismatch).
> 9. **Δ9 (§4.8)** — `_utcnow()` hoisted outside the per-text loop. All texts from one record share one `extracted_at`. Cleaner semantic.
> 10. **Δ10 (§4.8)** — registry SQL column is `connector` (not `connector_id`). Deployed `cip_08_tickets_and_registry.py` uses column name `connector`; ON CONFLICT key matches.
> 11. **Δ11 (§4.8)** — registry SQL column is `property_type` (not `data_type`). `PropertyDescriptor.data_type` Python attribute preserved (no public-API break); SQL bind layer maps `p.data_type` → `:property_type`.
> 12. **Δ12 (§4.1 docstring)** — `PropertyDescriptor.data_type` docstring enumerates the deployed CHECK enum: `{string, number, datetime, enumeration, reference, boolean, array, object}`. Pre-v5.4 docstring listed open values like `"integer"`, `"timestamp"`, `"json"` that are NOT in the deployed enum.
> 13. **Δ13 (§5.1.1)** — `DELTA_CONTACTS` distinct-source_id assertion text/count. Pre-v5.4 plan said the delta has 2 distinct source_ids (treating mutated-c003 as an intra-batch duplicate). Reality: 3 distinct source_ids; c003 is a cross-corpus mutation that exercises the SCD-2 archive path; 2 are new not in baseline. Assertion text + count both fixed.
> 14. **Δ14 (§4.4)** — `apply_tenant_context()` SQL flipped from `text("SET LOCAL app.current_tenant = :tid")` to `text("SELECT set_config('app.current_tenant', :tid, true)")`. Postgres `SET LOCAL` does not accept bind parameters; the deployed framework uses the documented Postgres equivalent. Mock-based unit tests didn't catch this because `MagicMock.execute()` doesn't parse SQL; the dry-run against a real Postgres testcontainer surfaced it pre-§5 cascade.
>
> **Plus 3 plan-text fixes:**
>
> - **Item 16** — Conformance test count is **7**, not 6. v4 PATCH-NR-1 (Round-3 panel CRIT-1 mitigation) added `test_post_commit_rls_isolation`. SPEC §11 still enumerates six (the original Round-1 design); v5.4 disambiguates throughout: "the seven conformance tests" = deployed harness; "the six original SPEC tests" = SPEC's count. Updated in §0 TL;DR, §1 reference, §3 file layout comment, §5 header, §5.1.1 (×2), §6 unit tests, §9 #4, §10.1 §11.
> - **Item 17** — §10.1 doc-fill scope now mentions `MAX_RATE_LIMIT_SLEEP_SECONDS=300` cap in connector authoring guide §10. Connector authors must know the orchestrator caps `RateLimitExceeded.retry_after_seconds` at 300 (a buggy connector cannot park a sync indefinitely).
> - **Item 18** — §10.2 doc-fill scope now documents the deployed-5 counter mapping (Δ2), `set_config(...)` semantics (Δ14), `autoflush=False, expire_on_commit=False` rationale, and Δ8 detect-then-assign for orchestrator-owned metadata keys.
>
> **Plus 1 acceptance-criterion retirement:**
>
> - **Item 19** — §9 #9 (`scripts/check_registry_sync.py` CSS drift check) **RETIRED in v5.4**. The script lived in the monorepo pre-D-152 extraction; it was not ported to foundry-cip. The substantive registry validation is now exercised by `tests/fixtures/connector_conformance/test_property_registry.py` against real Postgres. CSS classification headers are validated by Foundry's CI (the cross-pollution-guard job).
>
> **Item 15 (helper extraction) is NOT in v5.4.** The proposal to extract `_RLS_TEST_ROLE` + `_ensure_rls_test_role` to a shared `tests/_helpers/rls.py` module is a code refactor across both repos (monorepo + foundry-cip), not plan-hygiene. Captured as a separate ticket-style item, deferred.
>
> **Out of scope for v5.4:** all framework code in foundry-cip (already shipped + green); all architectural decisions; D-numbers; SPEC-level changes; the optional v5.4 plan-hygiene helper extraction (item 15).
>
> **Status:** plan-hygiene only. M2 itself is functionally complete (gate_pass decision `4757d300` recorded 2026-05-06). v5.4 exists so the plan document matches what was actually built; future readers reading the plan post-hoc should not have to reconstruct the 14 reconciliations from CC's exit reports.
...
3. A **seven-test conformance harness** that validates any connector+mapper pair meets the contract: protocol compliance, incremental sync, property registry populated, SCD history, sync-run audit, tenant scoping, **plus post-commit RLS isolation** (v4 PATCH-NR-1 added the 7th — see §5.8).
...
1. `products/client-intelligence-platform/vision/PHASE-1-PLAIN-SPEC.md` — §3 binding file paths, §4 binding Protocol shapes, §5 FixtureConnector data shape, §11 conformance tests (SPEC enumerates six; v4 PATCH-NR-1 added a 7th — `test_post_commit_rls_isolation` — making the deployed harness seven tests), §12 milestone order. This doc is **normative**. If this plan conflicts with the SPEC, the SPEC wins.
...
4. If changed → copy current row to `cip_{entity}_history` via bitemporal SCD-2 shape (fresh `history_id`, `record_id` FK to current row, `valid_from` = prior row's `refreshed_at`, `valid_to` = `now()`, `changed_by` = connector_id, `change_reason` = NULL in M2). Update current row with new values + new `previous_version_id` pointer. Increment `rows_updated` + `rows_history`. **v5.4 Δ3 (2026-05-06):** pre-v5.4 plan used a single `archived_at` timestamp on history rows; deployed reality is bitemporal. See §4.5 for the SQL pattern.
...
│   ├── fixtures/
│   │   └── connector_conformance/          M2 — seven conformance tests (v4 PATCH-NR-1 added test 7)
│   │       ├── __init__.py
...
**v5 PATCH-NR-7 (2026-04-29):** Add a `_assert_tz_aware(value: object, field_name: str) -> None` helper to base.py; raises `TimezoneNaiveError` if `value.tzinfo is None or value.utcoffset() is None`. Called by orchestrator before metadata is finalized into `KnowledgeText` and by persister before INSERT on any datetime field of `CIPRow.fields`. Same exception class as the existing incremental_key tz-naive check (line ~1412 of v4).

**v5.4 reconciliation Δ1 (2026-05-06):** Signature widened from `value: datetime` → `value: object`. The helper is called from `dict[str, object]` callsites (KnowledgeTextMetadata, CIPRow.fields), and a narrow `datetime` annotation makes mypy strict flag the runtime `isinstance` guard as unreachable. With `value: object`, the runtime `isinstance(value, datetime)` check is meaningful and the helper preserves no-op semantics on non-datetimes. Behavior preserved.
...
def _assert_tz_aware(value: object, field_name: str) -> None:
    ...
    v5.4 (Δ1): signature is `value: object` so callsites passing
    `dict[str, object]` values (KnowledgeTextMetadata, CIPRow.fields) type-check
    cleanly under mypy strict. The runtime isinstance guard does the actual
    type narrowing; non-datetime values are a no-op.
    """
    if not isinstance(value, datetime):
        return  # no-op for non-datetimes; callers may pass dict[str, object] values
...
class PropertyDescriptor:
    ...
    data_type: str             # Deployed CHECK enum (cip_08_tickets_and_registry.py):
                               #   {"string", "number", "datetime", "enumeration",
                               #    "reference", "boolean", "array", "object"}.
                               # v3: original v3 comment said this field was renamed
                               # from `property_type` to match a presumed SQL column
                               # name `data_type`. v5.4 Δ11 (2026-05-06): the deployed
                               # SQL column is actually `property_type`, not `data_type`.
                               # Python attribute name STAYS `data_type` (don't break
                               # the PropertyDescriptor public API). The persister maps
                               # `p.data_type` → SQL bind `:property_type` against the
                               # `property_type` column at write time. v5.4 Δ12: the
                               # docstring previously listed open values like "integer",
                               # "timestamp", "json" — those are NOT in the deployed
                               # CHECK enum. Connector authors must emit values from
                               # the 8-value set above.
...
### 4.4 `cip/integration_mesh/tenant_context.py` [v2: used by persister + recorder; v5.4 Δ14: SET LOCAL → set_config]

**Purpose:** Apply `SELECT set_config('app.current_tenant', :tid, true)` at the start of every batch transaction so RLS policies `cip_tenant_scope` work correctly. (v5.4 Δ14 — see below.)
...
def apply_tenant_context(db: SessionOrConnection, tenant_id: UUID) -> None:
    ...
    v4 (Round-3 panel CRIT-1) — pooler safety:
        set_config(..., true) — the third arg `is_local=true` — gives transaction-
        local scope, equivalent to `SET LOCAL`. As soon as the transaction commits
        or rolls back, the GUC is cleared. This is intentional and is what makes
        the call safe under PgBouncer TRANSACTION pooling — the connection is
        returned to the pool with no residual tenant state.
    ...
    v5.4 reconciliation Δ14 (2026-05-06):
        Original plan used `text("SET LOCAL app.current_tenant = :tid")`.
        Postgres `SET LOCAL` does NOT accept bind parameters — it requires a
        literal value. With a parameterized bind, psycopg sends `SET LOCAL
        app.current_tenant = $1`, which Postgres rejects as a syntax error.
        The deployed framework uses `SELECT set_config('app.current_tenant',
        :tid, true)` — the documented Postgres equivalent that accepts bind
        parameters. The third arg `true` (`is_local`) gives transaction-local
        scope matching `SET LOCAL` semantics. Mock-based unit tests didn't
        catch this because MagicMock.execute() doesn't parse SQL; the dry-run
        against a real Postgres testcontainer surfaced it pre-§5 cascade.
    """
    db.execute(
...
**Acceptance criteria:**
- v5.4 (Δ14): Uses parameterized `text()` with `SELECT set_config('app.current_tenant', :tid, true)` — the third-arg `is_local=true` gives transaction-local scope matching `SET LOCAL` semantics. (Pre-v5.4 plan said `SET LOCAL :tid` which Postgres rejects.)
...
### 4.5 `cip/integration_mesh/persister.py` [v2: ALLOWED_CIP_TABLES allowlist + SQLAlchemyError translation; v5.4 Δ3+Δ4+Δ5+Δ6: bitemporal SCD-2 + per-table extras + cip_views guard + IS NOT DISTINCT FROM]
...
**v5.4 reconciliations (2026-05-06) — four schema-deltas surfaced when M2 framework code was built against the deployed `cip_01_clients.py` through `cip_08_tickets_and_registry.py` migrations:**

- **Δ3 (bitemporal SCD-2 history shape):** Plan §4.5 (pre-v5.4) used a single `archived_at` timestamp + a copy of `id` + `client_id` snapshot on history rows. Deployed reality (all 7 `cip_*_history` tables): bitemporal SCD-2 with `record_id` (FK to `<current>.id`), `valid_from NOT NULL`, `valid_to` NULL-allowed, `changed_by NOT NULL`, `change_reason` NULL-allowed, plus a `valid_to IS NULL OR valid_to > valid_from` check constraint. No `id` column on history; no `client_id` snapshot; no `archived_at`. The history INSERT-SELECT pattern below reflects the deployed shape. Per P-22 / D-123, migrations are the authority — and the bitemporal shape is genuinely better than the plan's single-timestamp model (matches Kimball SCD-2 norms).
- **Δ4 (per-table extras column):** Plan §4.5 used a uniform `overflow`/`properties` JSONB column across all 7 tables. Deployed reality is heterogeneous: `cip_clients` uses `metadata`, `cip_views` has NO extras column, the other 5 (`cip_files`, `cip_contacts`, `cip_companies`, `cip_deals`, `cip_tickets`) use `properties`. Persister carries a module-level `EXTRAS_COLUMN_BY_TABLE: dict[str, str | None]` mapping; INSERT/UPDATE consult it on every write to bind `CIPRow.overflow` against the actual column name (with the column name varying per target).
- **Δ5 (cip_views overflow guard):** Plan §4.5 assumed every domain table has an extras column. Deployed reality: `cip_views` has only `filter_config JSONB` (a domain column for view-filter semantics), no generic extras column. Persister raises `PersistenceError(f"table {target} has no overflow column but mapper produced overflow keys: ...")` when `EXTRAS_COLUMN_BY_TABLE[target] is None` and `row.overflow` is non-empty. M2's MockMapper doesn't write `cip_views`, so this is dead-path for now; the guard catches mapper bugs the moment a real connector hits it.
- **Δ6 (IS NOT DISTINCT FROM for nullable natural keys):** Plan §4.5 SQL (pre-v5.4) used `WHERE source_id = :source_id` for the SCD lookup (current-row-by-natural-key). Deployed reality: `cip_files.source_id` is `nullable=True` (R2-uploaded blobs without source provenance); the other 6 are `NOT NULL`. With nullable column, `source_id = :source_id` is always-false on NULL rows — silent miss. v5.4 uses `source_id IS NOT DISTINCT FROM :source_id` uniformly across all 7 tables (the standard Postgres idiom for SCD lookup against nullable natural keys). The `(tenant_id, source_connector, source_id)` index handles either way; uniform application beats per-table specialization.
...
# v5.4 Δ4 + Δ5 (2026-05-06): per-table extras column mapping. Deployed schema
# is heterogeneous — see migrations cip_03 (clients = `metadata`), cip_04
# (views — no extras column at all), cip_05/06/07/08 (files, contacts,
# companies, deals, tickets = `properties`). Persister consults this on every
# INSERT/UPDATE to bind CIPRow.overflow against the deployed column name.
# A None value means "no extras column on this table"; persister raises
# PersistenceError if a mapper produces overflow keys for such a table.
...
class CIPRowPersister:
    ...
    def persist(
        ...
        *,
        ...
        # v5.4 Δ5: cip_views has no extras column. Mapper-emitted overflow
        # keys against such a table are a bug — fail loud the moment a real
        # connector hits the path. M2 MockMapper writes cip_contacts so the
        # check is dead-path for the harness; it catches the future case.
        extras_column = EXTRAS_COLUMN_BY_TABLE.get(row.target_table)
        if extras_column is None and row.overflow:
            raise PersistenceError(
        ...
        try:
            ...
            #    v5.4 Δ6: lookup uses IS NOT DISTINCT FROM for nullable source_id
            #    on cip_files. Uniform across all 7 tables.
            ...
            #      history_table (bitemporal SCD-2 — Δ3); UPDATE current;
            #      return updated=1, history=1.
...
For step 2 (INSERT new) — v5.4 Δ4: extras column name varies per table:
```sql
-- For tables WITH an extras column (cip_clients/files/contacts/companies/deals/tickets):
INSERT INTO {row.target_table} (
    ...
    {EXTRAS_COLUMN_BY_TABLE[row.target_table]}   -- "metadata" for cip_clients;
                                                  -- "properties" for the other 5
) VALUES (
    gen_random_uuid(), :tenant_id, :client_id, :source_connector, :source_id,
    now(), now(),
    NULL, :batch_id, :authority,
    {:field_1, :field_2, ...},
    CAST(:_extras AS jsonb)
)
RETURNING id

-- For cip_views (no extras column — Δ5): the same INSERT minus the extras
-- column entirely. Mapper emitting non-empty overflow on cip_views was already
-- rejected at the top of persist() by the Δ5 guard.
```

For step 3 (UPDATE with bitemporal history — Δ3 + Δ6):
```sql
...
-- in the SAME order, preventing the deadlock class.
-- v5.4 Δ6: IS NOT DISTINCT FROM for nullable source_id (cip_files).
SELECT id, {all domain columns}, {EXTRAS_COLUMN_BY_TABLE[target] AS extras_or_omitted}
FROM {row.target_table}
WHERE tenant_id = :tenant_id
  AND source_connector = :source_connector
  AND source_id IS NOT DISTINCT FROM :source_id   -- Δ6: NULL-equating for cip_files
ORDER BY source_id
FOR UPDATE

-- 3b. If differ says changed AND should_write_history (Δ3 — bitemporal SCD-2):
--     The history row snapshots the SCD-relevant columns (record_id JOIN
--     reconstructs client_id from current; valid_from = the moment this revision
--     became current = the prior row's refreshed_at; valid_to = now()).
INSERT INTO {row.target_table}_history (
    history_id,        -- gen_random_uuid()
    record_id,         -- FK back to {row.target_table}.id
    tenant_id,
    source_connector,
    source_id,
    valid_from,        -- the prior row's refreshed_at (when this revision became current)
    valid_to,          -- now() (when this revision stops being current)
    changed_by,        -- :connector_id (who archived it; M2 has no human reviewers)
    change_reason,     -- NULL in M2; Phase 3+ may populate (e.g., 'agent_inferred', 'human_corrected')
    {domain columns copied from prior row by name},
    {EXTRAS_COLUMN_BY_TABLE[target] copied from prior row, if present}
)
SELECT
    gen_random_uuid(),
    id,                                  -- record_id ← prior row's id
    tenant_id,
    source_connector,
    source_id,
    refreshed_at,                        -- valid_from ← prior row's refreshed_at
    now(),                               -- valid_to ← now
    :changed_by,                         -- bound to connector_id
    NULL,                                -- change_reason
    {domain columns},
    {extras column if any}
FROM {row.target_table}
WHERE id = {prior row's id}

-- 3c. UPDATE current (with previous_version_id pointer to the new history row):
UPDATE {row.target_table}
SET refreshed_at = now(),
    previous_version_id = {history_id we just generated},
    {domain columns = new values},
    {EXTRAS_COLUMN_BY_TABLE[target]} = CAST(:_extras AS jsonb),  -- skip on cip_views
    ingestion_batch_id = :batch_id
WHERE id = {current id}
```

**Note on column ordering:** the `SELECT … INSERT INTO history` pattern above relies on `_get_table_columns(history_table)` returning columns in declaration order (Postgres preserves this). The persister's runtime reflection (`sa.inspect(engine).get_columns(history_table)`) is stable across runs.

**Edge cases:**
- JSON/extras diff: differ compares via canonical JSON serialization (sorted keys) to avoid false positives from key-order changes.
...
- Very large extras JSONs: no special handling in M2; rely on Postgres JSONB to handle. Flag for M6 if we see real-world extras >1MB.
- v5.4 Δ5 — `cip_views` overflow guard: persister raises `PersistenceError` BEFORE any SQL runs if a mapper produced non-empty `row.overflow` for `cip_views`. M2's MockMapper doesn't hit this; v5.4 documents the guard.
- v5.4 Δ6 — nullable `cip_files.source_id`: `IS NOT DISTINCT FROM` is the standard Postgres idiom; uniform across all 7 tables for query-path simplicity.
...
- v5.4 Δ3: history INSERT-SELECT writes bitemporal SCD-2 columns (`record_id`, `valid_from`, `valid_to`, `changed_by`, `change_reason`) into the deployed `cip_{entity}_history` shape; verified by `tests/fixtures/connector_conformance/test_scd_history.py`.
- v5.4 Δ4: persister consults `EXTRAS_COLUMN_BY_TABLE` on every INSERT/UPDATE; binds `CIPRow.overflow` against the deployed column name (`metadata` for cip_clients, `properties` for the 5 standard tables).
- v5.4 Δ5: persister raises `PersistenceError` on overflow against `cip_views` (no extras column); verified by unit test in `tests/integration_mesh/test_persister.py`.
- v5.4 Δ6: SCD lookup SQL uses `source_id IS NOT DISTINCT FROM :source_id`; verified by unit test against a `cip_files` row with NULL source_id.
- Returns `PersistResult` with correct counts.
- Raises `PersistenceError` on DB errors (IntegrityError, etc.) with context.

**Test pointer:** `tests/fixtures/connector_conformance/test_scd_history.py` (history shape, multi-revision walk via `previous_version_id`); `tests/integration_mesh/test_persister.py` (Δ4/Δ5/Δ6 unit verification + bitemporal column-list shape).
...
### 4.7 `cip/integration_mesh/sync_run_recorder.py` [v2: owns its own connection, SET LOCAL per recorder write; v5.4 Δ2: deployed 5-counter shape]

**Purpose:** Manage the `cip_sync_runs` row lifecycle as a context manager so the orchestrator can't forget to close the row.

**v5.4 reconciliation Δ2 (2026-05-06):** Pre-v5.4 plan §4.7 specified 7 SQL counter columns + a `run_metadata` column rename, gated on two new migrations `cip_11_rename_sync_runs_metadata.py` and `cip_12_sync_runs_counter_split.py` to be authored before M2 ships. Deployed `cip_03_sync_runs.py` ships a 5-counter shape: `rows_ingested`, `rows_history`, `rows_created`, `rows_updated`, `rows_skipped` + a `metadata` column (no rename). The migrations cip_11 / cip_12 were never authored. v5.4 reconciliation: keep the granular 7 counters in the in-memory `_MutableCounters` (orchestrator uses them for structured logging), and map them to the deployed 5 at SQL write time. The mapping:
- `rows_ingested = rows_created + rows_updated` (deployed counter combines them)
- `rows_skipped = rows_skipped_unchanged + rows_skipped_drift + rows_skipped_duplicate` (deployed counter combines)
- `rows_created`, `rows_updated`, `rows_history` → 1:1
- `rows_received` is in-memory only (no SQL slot — granularity collapses)
- INSERT writes the deployed `metadata` column name (not `run_metadata`)

Per P-22 / D-123, migrations are the authority. The pre-v5.4 plan's R2-A3 fallback path explicitly authorized this ("If renaming is blocked, fall back to keeping `metadata` column name here and documenting the footgun"); v5.4 makes the fallback canonical.

**v2 redesign (QC C-1, C-2, Senior #1, #2):** Recorder takes a SQLAlchemy `Engine`, not a `Session`. It opens a fresh short-lived connection for each of its two writes (INSERT on enter, UPDATE on exit). Each connection does its own `BEGIN → SELECT set_config('app.current_tenant', ...) → write → COMMIT` inside `engine.begin()`. This means:
...
class SyncRunRecorder:
    ...
    def __enter__(self) -> "SyncRunRecorder":
        ...
        # set_config inside this txn so RLS on cip_sync_runs passes (QC C-2 fix).
        # engine.begin() auto-commits on exit; the cip_sync_runs row becomes
        # observable immediately ("watch sync live" operator UX).
        # v5.4 Δ2: INSERT uses deployed column name `metadata` (not `run_metadata`).
        ...
        return self
        # v5.4 Δ2 (2026-05-06): the migrations cip_11_rename_sync_runs_metadata
        # and cip_12_sync_runs_counter_split were planned (R2-A3) but never
        # authored. Deployed cip_03_sync_runs uses the column `metadata` (not
        # `run_metadata`) and a 5-counter shape (rows_ingested, rows_history,
        # rows_created, rows_updated, rows_skipped). v5.4 plan reconciliation:
        # accept the deployed shape as canonical, retain the granular 7 in
        # the in-memory _MutableCounters for structured logging, and map at
        # write time (see __exit__ below). No new migrations gate M2.
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            # v5.4 Δ2: UPDATE binds against the deployed 5-counter shape:
            # rows_ingested = created + updated;
            # rows_skipped = skipped_unchanged + skipped_drift + skipped_duplicate;
            # rows_created/rows_updated/rows_history → 1:1.
            # rows_received is in-memory only (no SQL slot).
            with self.engine.begin() as conn:
                apply_tenant_context(conn, self.tenant_id)
                conn.execute(
                    text(
                    ...
                    ),
                    {
                        "status": status,
                        "ended_at": ended_at,
                        # v5.4 Δ2 mapping: in-memory granular 7 → SQL deployed 5.
                        "rows_ingested": (
        ...
        # v5.4 Δ2 (2026-05-06): the planned counter-split migration cip_12 was
        # never authored. Deployed cip_sync_runs ships rows_ingested/rows_history/
        # rows_created/rows_updated/rows_skipped (5 counters). The orchestrator's
        # in-memory _MutableCounters keeps the granular 7 for structured logging;
        # __exit__ collapses them to the deployed 5 at write time. Mapping above.
        # The granular 7 stay observable to operators via app logs (one log line
        # per batch with the counter delta).
...
### 4.8 `cip/integration_mesh/orchestrator.py` [v2: single stream_records call, batch-scope cursor writes, safety-window, rate-limit budget; v5.4 Δ7+Δ8+Δ9+Δ10+Δ11]
...
**v5.4 reconciliations (2026-05-06) — five orchestrator-pattern fixes surfaced during framework build:**

- **Δ7 (_utcnow helper):** Pre-v5.4 plan referenced `_utcnow()` inside the metadata-finalize block but never defined it. v5.4 defines `_utcnow()` as an orchestrator-private helper returning `datetime.now(UTC)`. Tests can monkeypatch `cip.integration_mesh.orchestrator._utcnow` for deterministic time.
- **Δ8 (setdefault semantics split):** Pre-v5.4 plan used `setdefault` uniformly for all 5 required keys + `ingestion_batch_id`. The 5 required keys split into two authority classes: 3 are mapper-may-know-better (`source_system`, `connector_version`, `extracted_at` — mapper has legitimate cases to override), 2 are orchestrator-owned (`tenant_id`, `ingestion_batch_id` — mapper has no business emitting). v5.4 keeps `setdefault` for the 3 mapper-authority keys; uses **detect-then-assign** for the 2 orchestrator-owned keys (raise `KnowledgeMetadataValidationError` if mapper emitted a different value, then assign). Silent overwrite would mean a misbehaving mapper never learns about its bug; fail-loud is the framework philosophy.
- **Δ9 (_utcnow hoisted outside per-text loop):** Pre-v5.4 plan called `_utcnow()` inside the `for t in raw_texts:` loop, producing N microsecond-different timestamps for one record's N KnowledgeTexts. v5.4 hoists `extraction_ts = _utcnow()` outside the inner loop; all texts from one record share one `extracted_at`. Cleaner semantic ("when did the framework cross this record into knowledge?") and easier to reason about.
- **Δ10 (`connector` not `connector_id` in registry SQL):** Pre-v5.4 plan's `_register_properties_best_effort` INSERT used column name `connector_id` and `ON CONFLICT (tenant_id, connector_id, object_type, property_name)`. Deployed `cip_08_tickets_and_registry.py` uses column `connector` and `ON CONFLICT (tenant_id, connector, object_type, property_name)`. `PropertyDescriptor.connector` Python field already aligns with the SQL column.
- **Δ11 (`property_type` not `data_type` in registry SQL):** Pre-v5.4 plan's INSERT used column name `data_type`. Deployed schema uses `property_type` with a CHECK enum constraint. `PropertyDescriptor.data_type` Python attribute STAYS (don't break the public API); only the SQL bind layer adapts (`p.data_type` → `:property_type` binds to the `property_type` column). See §4.1 Δ12 for the docstring-level enum reconciliation.
                    ...
                    try:
                        ...
                        with Session(engine, autoflush=False, expire_on_commit=False) as db, db.begin():
                            ...
                            for rec in batch:
                                ...
                                #
                                # v5.4 Δ7 + Δ8 + Δ9 (2026-05-06):
                                # - Δ7: `_utcnow()` defined as orchestrator-private helper
                                #       (see helpers section below).
                                # - Δ8: setdefault for 3 mapper-may-know-better keys
                                #       (source_system, connector_version, extracted_at);
                                #       detect-then-assign for 2 orchestrator-owned keys
                                #       (tenant_id, ingestion_batch_id) — fail-loud on
                                #       mapper-emitted mismatches.
                                # - Δ9: extraction_ts hoisted outside the per-text loop
                                #       so all texts from one record share one extracted_at.
                                try:
                                    raw_texts = mapper.ingest_as_knowledge(rec)  # list[KnowledgeText]
                                    if raw_texts:
                                        # Δ9: per-record extraction timestamp (one ts shared
                                        # across all texts from this record).
                                        extraction_ts = _utcnow()
                                        finalized_texts = []
                                        for t in raw_texts:
                                            md = dict(t.metadata)
                                            # Δ8 — mapper-may-know-better keys: setdefault.
                                            ...
                                            # Δ8 — orchestrator-owned keys: detect-then-assign.
                                            mapper_tenant_id = md.get("tenant_id")
                                            if mapper_tenant_id is not None and mapper_tenant_id != tenant_id:
                                                raise KnowledgeMetadataValidationError(
                                                    f"mapper emitted tenant_id={mapper_tenant_id} but run is "
                                                    f"bound to tenant_id={tenant_id} — orchestrator-owned key "
                                                    f"cannot be overridden",
                                                )
                                            md["tenant_id"] = tenant_id
                                            mapper_batch_id = md.get("ingestion_batch_id")
                                            if mapper_batch_id is not None and mapper_batch_id != run.batch_id:
                                                raise KnowledgeMetadataValidationError(
                                                    f"mapper emitted ingestion_batch_id={mapper_batch_id} but "
                                                    f"run batch_id={run.batch_id} — orchestrator-owned key "
                                                    f"cannot be overridden",
                                                )
...
def _utcnow() -> datetime:
    """v5.4 Δ7 (2026-05-06): orchestrator-private current-UTC helper.

    Defined here so tests can monkeypatch
    `cip.integration_mesh.orchestrator._utcnow` for deterministic time without
    monkeypatching `datetime.now` globally. Pre-v5.4 plan referenced
    `_utcnow()` in the metadata-finalize block but never defined it.
    """
...
def _register_properties_best_effort(
    ...
    v5.4 Δ10 + Δ11 (2026-05-06): SQL column names match deployed
    cip_connector_property_registry: `connector` (not `connector_id`) and
    `property_type` (not `data_type`). PropertyDescriptor's Python attribute
    `data_type` is preserved (don't break the public API); the SQL bind
    layer maps `p.data_type` → `:property_type` against the deployed column.
    """
    try:
                ...
                db.execute(
                    sa.text(
                        ...
                        INSERT INTO cip_connector_property_registry (
                            ...
                            property_type, is_custom, storage_location, column_name,
                            description
                        ) VALUES (
                            :tenant_id, :connector, :object_type, :property_name,
                            :property_type, :is_custom, :storage_location, :column_name,
                    ...
                    {
                        "tenant_id": tenant_id,
                        # v5.4 Δ10: bind name `:connector` matches deployed column.
                        "connector": connector.connector_id,
                        "object_type": p.object_type,
                        "property_name": p.property_name,
                        # v5.4 Δ11: PropertyDescriptor.data_type → bind `:property_type`.
                        # Python attr name preserved; SQL column name authoritative.
...
## 5. Conformance Harness — Seven Tests (§SPEC §11 + v4 PATCH-NR-1)

All seven live under `tests/fixtures/connector_conformance/`. The harness runs against a `MockConnector` + `MockMapper` defined in that folder's `conftest.py` (NOT a real FixtureConnector — that's M3). Each test is self-contained and parametrized over `(MockConnector, MockMapper)` fixtures so that when M3 lands, the same tests re-run against the real `FixtureConnector` with zero changes. **v5.4 note (2026-05-06):** SPEC §11 enumerates six tests (the original Round-1 design); v4 PATCH-NR-1 (Round-3 panel CRIT-1 mitigation) added the 7th — `test_post_commit_rls_isolation.py` — see §5.8. Throughout this plan, "the seven conformance tests" refers to the deployed harness shape; "the six original SPEC tests" refers to the SPEC's count.
...
Fixed corpus used across ALL seven conformance tests so the property registry + SCD + incremental-key paths all key off the same input. Lives in `tests/fixtures/connector_conformance/fixtures/records.py`:
...
# v5.4 Δ13 (2026-05-06): the third record (c003) is a CROSS-CORPUS mutation —
# it shares a source_id with a baseline record (CANONICAL_CONTACTS c003) but
# carries a NEW updated_at + email. It is NOT an intra-batch duplicate. The
# delta has 3 distinct source_ids; 2 of those are new not in baseline (c011,
# c012); c003 is the cross-corpus mutation that exercises the SCD-2 path.
DELTA_CONTACTS: list[dict] = [
    ...
    # Cross-corpus mutation of CANONICAL_CONTACTS c003 — exercises the SCD-2
    # archive path on second run. NOT an intra-batch duplicate.
...
Rationale for locking the corpus now: three of the seven conformance tests (incremental sync, property registry, SCD history) each construct their own inline records in v1, so regressions in one can pass while another silently uses a different shape. A single canonical corpus catches this.
...
assert len(DELTA_CONTACTS) == 3, "delta corpus must be 3 records (2 new + 1 cross-corpus mutation)"
# v5.4 Δ13 (2026-05-06): delta has 3 distinct source_ids (c011, c012,
# mutated-c003). Of those, 2 are new not in baseline (c011, c012). The third
# (c003) is a cross-corpus mutation, NOT an intra-batch duplicate.
assert len({r["source_id"] for r in DELTA_CONTACTS}) == 3, \
    "delta must contain exactly 3 distinct source_ids (c011, c012, mutated-c003)"
new_in_delta = (
    {r["source_id"] for r in DELTA_CONTACTS}
    - {r["source_id"] for r in CANONICAL_CONTACTS}
)
assert len(new_in_delta) == 2, \
    "exactly 2 source_ids in delta must be new not in baseline (c011, c012)"
...
# (10 baseline + 2 new; c003 was updated in place — SCD-2 archive path —
# not added as a new row, because it already existed).
```
...
- Mutate same record: `email=b@x.com`. Run sync again. Expect 1 row in `cip_contacts` (email=b), 1 row in `cip_contacts_history` snapshotting the prior revision (email=a) with bitemporal SCD-2 columns set: `record_id` = current row's id, `valid_from` = prior row's `refreshed_at`, `valid_to` = `now()`, `changed_by` = connector_id, `change_reason` = NULL. **v5.4 Δ3 (2026-05-06):** pre-v5.4 plan said `archived_at` is set; deployed schema uses the bitemporal columns above.
...
- Verify ordering: `cip_contacts_history` can be walked backwards through `previous_version_id` to reconstruct the full change log (full multi-revision walk is a v5.4-known partial — current test verifies first hop; recursive walk extension is v5.4 follow-on per CC §4.10+§5 exit notes).

### 5.6 `test_sync_run_audit.py`

- Success path: sync 5 records; assert `cip_sync_runs` row has `status='success'`, `rows_ingested=5, rows_created=5, rows_updated=0, rows_skipped=0` (deployed-5 SQL counter shape — Δ2), `started_at < ended_at`, `error_detail IS NULL`. **v5.4 Δ2 (2026-05-06):** the SQL row exposes the deployed-5 collapse; the orchestrator's in-memory `_MutableCounters` carries the granular 7 (`rows_received`, `rows_skipped_unchanged`, etc.) for structured logging. Tests asserting against `cip_sync_runs` rows must use the deployed-5 column names; tests asserting against the returned `SyncRunState` use the in-memory granular 7.
...
Beyond the 7 conformance tests, M2 ships targeted unit tests for internals:
...
| 4 | `pytest tests/fixtures/connector_conformance/` — all 7 tests pass against `MockConnector`. (v5.4 reconciliation: deployed count is 7, not 6 — v4 PATCH-NR-1 added `test_post_commit_rls_isolation`. The harness ships 22 conformance assertions across 7 test modules.) | `pytest tests/fixtures/connector_conformance/ -v` (≥7 PASSED, 0 FAILED, 0 SKIPPED — exact assertion count grows as the harness extends). |
...
| 9 | ~~`scripts/check_registry_sync.py` (CSS drift check) passes.~~ **RETIRED in v5.4 (2026-05-06).** The script lived in the monorepo pre-D-152 extraction; it was not ported to foundry-cip. The substantive registry validation it provided is now exercised by `tests/fixtures/connector_conformance/test_property_registry.py` (5-descriptor seed + idempotent upsert + `is_custom`-OR preservation) — the conformance assertion is stronger than the script's grep-style check because it runs against real Postgres. CSS drift on the monorepo side is still covered by the monorepo's own `check_registry_sync.py`; CIP's CSS classification headers are validated by Foundry's CI (the cross-pollution-guard job catches absence of `# foundry: kind=` headers). | N/A — RETIRED. |
...
**§10 Rate-limit policy:** Expose via `@property rate_limit_policy`. Default `RateLimitPolicy.DEFAULT` (10 rps, burst 5). Override per source-system docs. Orchestrator honors via TokenBucket. **v5.4 (item 17):** mention `MAX_RATE_LIMIT_SLEEP_SECONDS = 300` cap explicitly — connectors raising `RateLimitExceeded` with `retry_after_seconds > 300` will be capped to 300 by the orchestrator (a buggy/malicious connector cannot park a sync for an hour); after `MAX_BATCH_RATE_LIMIT_RETRIES = 3` consecutive `RateLimitExceeded` retries on the same batch, the batch counts as one consecutive-failure increment.

**§11 Passing the conformance harness:** Point to `tests/fixtures/connector_conformance/`. Must pass all 7 before merge (v5.4 reconciliation: SPEC §11 enumerates six tests; v4 PATCH-NR-1 added `test_post_commit_rls_isolation` — see §5.8).
...
**§4 cip_sync_runs lifecycle:** `status`: `running → {success, partial, failed}`. Fields: `id, tenant_id, client_id, connector_id, connector_name, batch_id, sync_mode, rows_ingested, rows_history, rows_created, rows_updated, rows_skipped, started_at, ended_at, cursor_state, error_detail, metadata`. Row is committed at start (observable during long syncs). **v5.4 (item 18 — Δ2 mapping):** the orchestrator's in-memory `_MutableCounters` carries the granular 7-counter view (`rows_received, rows_created, rows_updated, rows_skipped_unchanged, rows_skipped_drift, rows_skipped_duplicate, rows_history`); the recorder's `__exit__` UPDATE collapses the 7 → deployed 5 at SQL write time (`rows_ingested = rows_created + rows_updated`; `rows_skipped = rows_skipped_unchanged + rows_skipped_drift + rows_skipped_duplicate`; the other three map 1:1; `rows_received` is in-memory only). Operators wanting full granularity read structured logs; the cip_sync_runs row is the SQL-friendly aggregate.
...
**§6 Transaction boundaries:** One txn per batch. `BEGIN → apply_tenant_context → INSERT/UPDATE batch rows → COMMIT`. On error: ROLLBACK; orchestrator increments consecutive-failure count. Abort after 3. **v5.4 (item 18):** the per-batch Session uses `Session(engine, autoflush=False, expire_on_commit=False)` — explicitly disabling autoflush prevents the implicit-flush deadlock surface (Round-3 panel HIGH); explicitly disabling expire_on_commit prevents post-commit attribute-refresh round-trips against rows the orchestrator has already finished with. Both are cheap and remove a class of surprise. **v5.4 (Δ14):** `apply_tenant_context()` issues `SELECT set_config('app.current_tenant', :tid, true)` — the `is_local=true` third arg gives transaction-local scope matching `SET LOCAL` semantics. (The pre-v5.4 plan used `text("SET LOCAL ... = :tid")` which Postgres rejects because `SET LOCAL` does not accept bind parameters.)

**§8 Failure modes + partial sync:** Catalog from §8 above (auth, rate-limit, schema drift, persistence).

**§9 Observability:** Structured logs (module-level `log`). Per-batch DEBUG. Per-failure WARNING or ERROR. `cip_sync_runs` row is the primary operational artifact. **v5.4 (item 18 — Δ8 metadata authority):** the orchestrator finalizes `KnowledgeText.metadata` with two distinct authority modes — three keys (`source_system`, `connector_version`, `extracted_at`) use `setdefault` because mappers may legitimately know better; two keys (`tenant_id`, `ingestion_batch_id`) use **detect-then-assign** because they are orchestrator-owned. A mapper emitting a different `tenant_id` or `ingestion_batch_id` than the run's authoritative values raises `KnowledgeMetadataValidationError` — fail-loud catches the bug instead of silently overwriting it.
