...
- **Phase 5 — Chatbot Capability (Internal / Staff-Facing).** Provisional. Three stages: 5A Vision, 5B Architecture, 5C Implementation. First tenant Wayward, via grant. Intended consumers listed in §7g. Restated by D-239 as a thin client: no standalone service, no per-tenant deployment.
